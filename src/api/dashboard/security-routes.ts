...
const SecurityPolicySchema = z.object({
  ...
  max_session_age_hours: z
    .number()
    .int()
    .min(1)
    .max(24 * 90)
    .optional(),
  reauth_interval_minutes: z
    .number()
    .int()
    .min(5)
    .max(24 * 60)
...
const OutboundWebhookSchema = z.object({
  ...
  event_types: z.array(z.enum(OUTBOUND_WEBHOOK_EVENTS)).min(1).max(20),
  enabled: z.boolean().optional(),
...
export function registerSecurityRoutes({
  ...
  router.get(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      sendSuccess(res, {
        enforce_mfa_for_admin_actions: policy.enforce_mfa_for_admin_actions ?? false,
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      await prisma.orgSettings.upsert({
        ...
        create: {
          ...
          securityPolicy: {
            enforce_mfa_for_admin_actions: d.enforce_mfa_for_admin_actions ?? false,
            sso_enforced: d.sso_enforced ?? false,
            allowed_sso_domains: (d.allowed_sso_domains ?? [])
      ...
      await auditLogs.record({
        ...
        metadata: {
          enforce_mfa_for_admin_actions: d.enforce_mfa_for_admin_actions ?? false,
          sso_enforced: d.sso_enforced ?? false,
          allowed_sso_domains_count: (d.allowed_sso_domains ?? [])
            .map((v) => v.trim().toLowerCase())
  ...
  router.get(
    ...
    requirePermission(prisma, "manage_permissions"),
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      const subscriptions = await listOutboundWebhookSubscriptions(prisma, req.organizationId!);
      sendSuccess(res, {
        subscriptions,
  ...
  router.post(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      const current = await listOutboundWebhookSubscriptions(prisma, req.organizationId!);
      try {
        await assertSafeOutboundUrl(payload.url, {
          allowHttp: process.env.NODE_ENV !== "production",
          allowHttps: true,
          denyPrivateNetworks: process.env.NODE_ENV === "production",
      ...
      await saveOutboundWebhookSubscriptions(prisma, req.organizationId!, [...current, next]);
      await auditLogs.record({
        organizationId: req.organizationId!,
        actorUserId: req.userId!,
        category: "POLICY",
  ...
  router.delete(
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
      ...
      if (next.length === current.length) {
        sendError(res, 404, "outbound_webhook_not_found");
        return;
      }
