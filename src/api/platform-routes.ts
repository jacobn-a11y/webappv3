...
import { sendSuccess, sendError, sendBadRequest } from "./_shared/responses.js";
...
export function createPlatformRoutes(prisma: PrismaClient): Router {
  ...
  const requirePlatformOwner = async (req: Request, res: Response, next: NextFunction) => {
    const platformOwnerEmail = process.env.PLATFORM_OWNER_EMAIL ?? "";
    if (!platformOwnerEmail) {
      sendError(res, 500, "PLATFORM_OWNER_EMAIL is not configured");
      return;
    }
    ...
    if (email !== platformOwnerEmail) {
      sendError(res, 403, "Platform owner access required");
      return;
    }
    ...
    asyncHandler(async (req, res) => {
      ...
      await prisma.tenantDeletionRequest.update({
        ...
        data: {
          status: "APPROVED",
          approvedAt: new Date(),
          approvedById: (req as PlatformOwnerRequest).userId ?? null,
          scheduledDeleteAt,
        },
      });
    ...
    asyncHandler(async (req, res) => {
      ...
      await prisma.tenantDeletionRequest.update({
        ...
        data: {
          ...
          cancelledById: (req as PlatformOwnerRequest).userId ?? null,
        },
      });
